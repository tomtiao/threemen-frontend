function showListMobi(): void {
    const show_list_checkbox = document.querySelector('.show_list') as HTMLInputElement;
    const section_list = document.querySelector('.section_list') as HTMLUListElement;
    ...
    const removeClassCanceled = (): void => {
...
`
    ...
    section_links.forEach((ele) => {
        if (window.location.pathname.includes((ele as HTMLAnchorElement).href)) {
...
function insertStyleSheet(path: string, css: string[]): void {
    if (!path.endsWith('/')) {
        path += '/';
    }
...
function insertScript(path: string, js: string[]): void {
    if (!path.endsWith('/')) {
        path += '/';
    }
...
// resource_path: where is the resourse
// resource_name: an array filled with resourse filename
// func: how to insert resource
interface InsertFunc {
    (resource_path: string, resourse_name: string[]): void;
}

function insert(resource_path: string, resource_name: string[], func: InsertFunc): void {
